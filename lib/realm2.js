...
class DB {
    ...
    deleteObjectGraph(object, type) {
        let objectSchema = this.database.schema.find((objectSchema) => objectSchema.name == type);
        ...
        for (let propName in objectSchema.properties) {
            ...
            if (prop.type == 'list') {
                object[propName].forEach((listObject) =>
                    deleteObjectGraph(listObject, prop.objectType)
                );

                if (prop.type == 'object' && object[propName] != null) {
                    deleteObjectGraph(object[propName], prop.objectType);
                }
            }
        }
        this.database.write(() => {
